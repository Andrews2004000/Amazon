...
export const promoteUser: RequestHandler = async (req, res, next) => {
    const { _id: id, role } = req.body as Partial<IUser>;

    if (!id || !role) {
        throw new AppError('Please provide id and role in the body', 400);
    }
